"""
Seed a CLEAN, unambiguous environment for the unified Mermaid→run→sign→distribute
walkthrough. No feature code. Idempotent (reuse by template-name / site-code).

Creates (under study f29f2acc 'MK-6482'):
  * a fresh TEMPLATE pointing at the on-disk local DOCX (so document_create works with
    Azure OFF) and with NO workflow definition yet -> the builder opens for authoring.
  * a fresh, uniquely-named SITE + StudySite + SiteProfile (signatory = RECIPIENT),
    with NO agreement -> the create flow shows the template picker and the per-
    (study_site,type) duplicate guard won't fire.
Prints every id you need. Confirms the local DOCX exists. Does NOT publish any
workflow (you author it via Mermaid in the builder).
"""
import asyncio
import os
import sys
...
TEMPLATE_NAME = "ZZ Mermaid Walk (local DOCX)"
SITE_CODE = "ZZ-WALK"
SITE_NAME = "ZZ Mermaid Walk Site"
...
async def main():
    ...
    async with AsyncSessionLocal() as db:
        ...
        # 1) template (reuse by name) — local DOCX, active, type CDA (label only in unified)
        ...
        # 2) site (reuse by code) + study_site + profile
        ...
        print("=" * 80)
        print("MERMAID WALK — ENVIRONMENT READY")
        print("=" * 80)
        print(f"Study        : {study_name}   id={STUDY_ID}")
        print(f"Site         : {SITE_NAME}   code={SITE_CODE}   site_id={site_id}")
        print(f"study_site   : {study_site_id}   agreements_here={n_agree}  (must be 0)")
        print(f"Template     : {TEMPLATE_NAME}   id={template_id}")
        print(f"Workflow def : key={key}   exists={bool(has_def)}  (must be False -> builder opens)")
        print(f"Recipient    : {RECIPIENT}")
        print(f"Local DOCX   : {LOCAL_DOCX}  (present)")
        print("=" * 80)
        print("In the app: pick Study 'MK-6482' (the one that lists the site 'ZZ Mermaid Walk Site'),")
        print("then Site 'ZZ Mermaid Walk Site', Agreements -> it shows the template picker.")
